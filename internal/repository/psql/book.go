...
import (
	...
	"github.com/alexvlasov182/web-app/go-boo-app/internal/domain"
)
...
func (b *Books) Create(ctx context.Context, book domain.Book) error {
	_, err := b.db.Exec("INSERT INTO books (title, author, publish_date, rating) values ($1, $2, $3, $4)", book.Title, book.Author, book.PublishDate, book.Rating)
...
func (b *Books) GetByID(ctx context.Context, id int64) (domain.Book, error) {
	...
	err := b.db.QueryRow("SELECT id, title, author, publish_date, rating FROM books WHERE id=$1", id).Scan(&book.ID, &book.Title, &book.Author, &book.PublishDate, &book.Rating)
